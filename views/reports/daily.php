<?php
$pageTitle = 'Daily Financial Report';
...
<div class="flex-1 flex flex-col min-w-0 bg-slate-50">
    ...
    <main class="flex-1 p-4 sm:p-6 lg:p-8 max-w-5xl w-full mx-auto space-y-6">
        ...
        <div class="flex border-b border-slate-200 gap-4 overflow-x-auto text-sm font-semibold">
            <a href="<?= BASE_URL ?>/reports/daily" class="py-2.5 px-4 text-sky-600 border-b-2 border-sky-600">Daily Report</a>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
            <h2 class="text-lg font-bold text-slate-900">Daily Cash Movement Summary</h2>

            <form action="<?= BASE_URL ?>/reports/daily" method="GET" class="flex flex-wrap items-center gap-2">
                ...
                <input type="date" name="date" value="<?= e($date) ?>" class="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700">
        ...
        <div class="bg-gradient-to-r from-slate-900 via-slate-800 to-sky-950 p-6 sm:p-8 rounded-2xl text-white shadow-lg border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="text-xs text-slate-300 bg-white/10 p-3.5 rounded-xl border border-white/10 space-y-1.5 sm:text-right">
                <div>This Month Money In: <span class="font-bold text-emerald-400"><?= Format::currency($groupStats['total_month_in']) ?></span></div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 sm:p-8 space-y-6">
            <div class="text-center pb-6 border-b border-slate-100">
                <span class="text-xs font-bold uppercase tracking-wider text-slate-400">REPORT FOR DATE</span>
                <div class="text-2xl font-extrabold text-slate-900 mt-1"><?= Format::date($dailyReport['date']) ?></div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div class="p-6 rounded-2xl bg-emerald-50/70 border border-emerald-100 text-center space-y-2">
                    <span class="text-xs font-bold text-emerald-800 uppercase tracking-wider block">MONEY IN TODAY</span>
                    <div class="text-2xl sm:text-3xl font-extrabold text-emerald-600">
                        <?= Format::currency($dailyReport['money_in']) ?>
                    </div>
                </div>

                <div class="p-6 rounded-2xl bg-rose-50/70 border border-rose-100 text-center space-y-2">
                    <span class="text-xs font-bold text-rose-800 uppercase tracking-wider block">MONEY OUT TODAY</span>
                    <div class="text-2xl sm:text-3xl font-extrabold text-rose-600">
                        <?= Format::currency($dailyReport['money_out']) ?>
                    </div>
                </div>

                <div class="p-6 rounded-2xl bg-sky-50/70 border border-sky-100 text-center space-y-2">
                    <span class="text-xs font-bold text-sky-800 uppercase tracking-wider block">NET DAILY CHANGE</span>
                    <div class="text-2xl sm:text-3xl font-extrabold <?= $dailyReport['net_change'] >= 0 ? 'text-sky-600' : 'text-rose-600' ?>">
                        <?= $dailyReport['net_change'] >= 0 ? '+' : '' ?><?= Format::currency($dailyReport['net_change']) ?>
                    </div>
                </div>
            </div>
        </div>
