    ...
    <main class="flex-1 p-4 sm:p-6 lg:p-8 max-w-3xl w-full mx-auto space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 sm:p-8">
            <div class="flex items-center gap-3 pb-5 border-b border-slate-100 mb-6">
                ...
                <div>
                    <h2 class="text-lg font-bold text-slate-900">Add New System User</h2>
                    <p class="text-xs text-slate-500">Create login credentials and assign 1 or more brands to manage.</p>
                </div>
            </div>
            ...
            <form action="<?= BASE_URL ?>/users/store" method="POST" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="email" id="email" name="email" required placeholder="john@group.com"
                               class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-slate-900 text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select id="role" name="role" required onchange="toggleBrandSelector()"
                                class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-slate-900 text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none">
                            <option value="brand_user">Brand User (Manage Assigned Brands)</option>
                            <option value="super_admin">Super Admin (Full CRUD System-Wide)</option>
                            <option value="manager">Manager (Read-Only System-Wide)</option>
                        </select>
                    </div>
                </div>

                <!-- MULTI-BRAND ASSIGNMENT SELECTOR -->
                <div id="brand_selector_container" class="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                    ...
                    <p class="text-xs text-slate-500 mb-3">This user will be permitted to add, edit, and manage financial data for the selected brands.</p>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        <?php foreach ($brands as $b): ?>
                            <label class="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-lg hover:border-purple-300 cursor-pointer transition-colors">
                                <input type="checkbox" name="brand_ids[]" value="<?= $b['id'] ?>" class="w-4 h-4 text-purple-600 rounded border-slate-300 focus:ring-purple-500">
                                <div>
